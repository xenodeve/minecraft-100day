...
## ปุ่มที่แพ็คตั้งมาให้แล้ว

แพ็คนี้ส่ง `options.txt` มาด้วยหนึ่งไฟล์ ที่มีแค่สี่บรรทัด และมันเปลี่ยน **keybind ของ Oculus สามตัว**
เท่านั้น ไม่แตะการตั้งค่าอื่นของคุณเลย

| ทำอะไร | ค่าเดิมของ Oculus | ของแพ็คนี้ | ทำไม |
|---|---|---|---|
| Reload shaders | `R` | **`F8`** | `R` คือปุ่ม reload ปืน TaCZ กดทีหนึ่ง shader คอมไพล์ใหม่ทั้งชุด ภาพกระตุกทุกครั้งที่เปลี่ยนแม็ก |
| Toggle shaders | `K` | **`F10`** | `K` มีสี่มอดจองอยู่ รวมถึงปุ่มเปิดเลเซอร์ของ TaCZ Additions |
| Shader pack selection | `O` | **`F12`** | `O` คือปุ่ม interact ของ TaCZ |

ถ้าอยากได้ปุ่มเดิมกลับมา เปลี่ยนได้ตามปกติที่ Options → Controls ค่าที่คุณเปลี่ยนจะอยู่ของมันเอง

> **สำหรับคนที่ดูแลแพ็ค:** ไฟล์นี้ตั้งใจให้ใส่ครั้งเดียวแล้วไม่แตะอีก เกมเขียนทับ `options.txt`
> ทุกรอบที่เล่น ไฟล์ของผู้เล่นจึงต่างจากของเราเสมอ และยังไม่ได้ยืนยันว่าการอัปเดตแพ็คจะทับ
> ค่าของเขาหรือไม่ ถ้าจะแก้ไฟล์นี้ทีหลัง ให้ถือเป็นการตัดสินใจที่ต้องเขียนผลกระทบกำกับไว้ (#123)

> **ทำไมไม่ใช่ F9** `Alt+F9` คือ hotkey อัดวิดีโอของ NVIDIA Overlay ตามค่าเริ่มต้น กดเปล่า ๆ
> ไม่ได้ชนกันจริง แต่กันปุ่มนั้นไว้ให้เลย ส่วน `Alt+F10` กับ `Alt+F12` ก็เป็นของ NVIDIA เหมือนกัน
> สองปุ่มนั้นปล่อยไว้เพราะไม่เหลือปุ่ม function ที่ว่างพอจะหลบได้ทั้งย่าน (#125)
