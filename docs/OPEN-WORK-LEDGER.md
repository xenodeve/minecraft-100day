...
| **Fix the GPU the client binds to** | 🔴 **UNTRACKED** | a Windows/NVIDIA setting, not a pack change | **#109**, conflicts `C-UPFG-07`. `latest.log` reads `OpenGL Renderer: NVIDIA GeForce RTX 5060 Ti/PCIe/SSE2` while an RTX 4070 SUPER is present. **Every performance measurement is blocked on this** — two runs on different GPUs are not comparable. Verify with `grep "OpenGL Renderer" latest.log`, never by feel |
