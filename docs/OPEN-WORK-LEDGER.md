...
| **Redistribution licences** — `docs/distribution-licenses.md` | ⚠️ **audit done, decision open** | needs the developer's call on self-contained vs hosted, and 39 project pages read | **#53**. 54 of 99 are permissively licensed. The finding is not the licence list: two authors permit modpacks *on condition the jar is not rehosted*, which is exactly what ADR 0003's self-contained model does. Two of the four API opt-outs are MIT/MPL and two are All-Rights-Reserved |
