...
| **Visuals V5–V8** — decorative and worldgen prototypes | ⚠️ **installed, undecided** | **⛔ DO NOT CREATE A REAL WORLD YET** | **#76**. Biomes O' Plenty `19.0.0.96`, Regions Unexplored `F-0.5.6`, TerraBlender `3.0.1.10`, Countered's Terrain Slabs `4.0.2-beta`, Polytone `1.20-3.5.26` — all installed and pinned. **§14 says "prefer one biome expansion" and both are in.** That is the kitchen-sink geography §14 warns against, and it is harmless only because no world has been generated. §15 wants multiple seeds compared across Base / BOP / RU / both, and §33 wants worldgen settled **before** persistent play. V8 must pick one first |
