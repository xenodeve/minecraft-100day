...
| **Candidate A test sequence (`PERF-UPFG-046`)** | 🔴 **UNTRACKED** | needs the GPU fixed, then a baseline | **#109/#111**. Two artifacts are built: `-nvidia-upscaling-addon.zip` (31 MB, drop into the profile you already play) for trying it, and `-nvidia-upscaling.zip` (214 MB, clean profile) for the actual A/B. Shader OFF and ON separately, TaCZ artifacts, FG at 2× only. `C-UPFG-01/02/03` are open and unfilled |
