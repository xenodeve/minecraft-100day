...
| Re-evaluate `TaCZ: Accelerated` | 🟡 | needs a benchmark baseline | Resolved as a STATUS by #93: `PERF-TACZ-ACCELERATED` is `BENCHMARK-GATED CANDIDATE / NOT CORE YET`, with a written promotion path. v1 contradicted itself and v2 deleted the topic; neither is a status. What remains is the measurement |
...
| **ServerCore — decide whether it belongs on clients too** | 🔴 **UNTRACKED** | a Performance Spec judgment, not a bug | The same reasoning error as #88 applies (zero `assets/` does not mean a singleplayer world does not want it), but the consequence is performance, not gameplay, so it was deliberately left out of #88's scope. Its own `mods.toml` declares `side = "BOTH"`, so nothing blocks the move. Decide under *Performance Spec: `PERF-SERVER-SERVERCORE`* and benchmark before moving |
| **Shaders + a TaCZ gun kills the client in ~60 s** | **#117** | needs one launch with shaders OFF to confirm or kill it | `C9`. `OutOfMemoryError: Failed to resize buffer from 2146435072 to -2146435072` — an int overflow in an **off-heap** vertex buffer, not RAM pressure (heap was 2.4/4 GB at all six crashes, so more `-Xmx` cannot help). Traced from `latest.log`, not the crash report, which blames the wrong mods because it only captures the second failure on the OOM screen. Shader active in **7 of 7** crashing runs; the shaderless instance has not crashed since `C8`. **Not the #115 add-ons** — PFG was in 1 of 6 |
| **Wisteria — the real DLSS-G backend, and it cannot be installed** | **#119** | blocked upstream; nothing on our side | `C-UPFG-11`. Super Resolution registers only *automatic* placeholders, so its FG menu section is never built — Candidate A does **upscaling only**. The concrete DLSS-G backend is `IReallyWantToSleep/Wisteria` (0 releases, no LICENSE; a Forge 1.20.1 CI artifact exists and was read). Its `mods.toml` demands `super_resolution [0.9.1-alpha.2,)` and **that version does not exist on Modrinth for any loader**. Unblocked only by SR publishing `0.9.1-alpha.2` for Forge 1.20.1. Also: FG needs *Use Vulkan Presentation* ON + restart |
| **`options.txt` is write-once, and the boot test for it is unrun** | **#123** | needs one launch | The pack now ships a four-line `options.txt` moving Oculus's `reload`/`toggleShaders`/`shaderPackSelection` off `R`/`K`/`O` — `R` was recompiling the shader pack on every TaCZ gun reload. **Unverified:** that a partial `options.txt` leaves unnamed options at their defaults, and whether an update overwrites a player's settings. **Policy: ship once, do not edit.** A later edit is a decision with that consequence, not a config tweak |
| **Flywheel's backend is off — Create draws on the vanilla path** | **#127** | one boot with shaders off answers the open half | `C10`. Crash report says `Flywheel Backend: flywheel:off` while `flywheel-client.toml` says `backend = "DEFAULT"` — **Flywheel disabled itself**, because Oculus is present and nothing bridges Iris↔Flywheel. Explains "base 80 FPS / field 180 FPS" better than any missing mod does, and it is a cost **we** introduced with the shader loader. **Unknown:** whether it is off from Oculus being installed or only while a shaderpack is in use. Check with `grep "Flywheel Backend"` after a shaders-off boot. Five candidates (Colorwheel, Particle Core, CreateBetterFps, Radium, C2ME-Forge) are recorded **DISCOVERED**, none installed — `PERF-FREEZE` holds and there is still no baseline |
| **Colorwheel cannot work until a supported shader pack is chosen** | **#133** | one settings change, or one download | `C12`. Colorwheel is installed and Flywheel is **still** `flywheel:off` — it reports `Shrimple_v0.11.zip is not compatible` in the log. **None of the six installed packs is a supported version.** Options: Complementary Unbound/Reimagined r5.7+ (no extra mod), or Colorwheel-Patcher + Bliss 2.1.1–2.1.2 / Photon 1.3a, or `enableFallbackMode = true` (author warns of graphical issues), or no shader. **Nothing measured** |
| **Run the fast attribution pass** | **#135** | two in-game sessions, ~1 hour | `benchmarks/FAST-PASS.md`. Same spot, shaders OFF then ON, `/spark profiler --thread * --timeout 60` each. Answers which thread is saturated and which mods own CPU on it, and settles `C9` and `C10`/`C12` from two grep lines. **`--thread *` is not optional** — `C13`: the previous profile sampled only the server thread, which is why it could not see the FPS problem. Analyse with `node scripts/analyze/spark-profile.mjs <code>` |
| **The benchmark world does not exist** | **#133** | the first real task now that the GPU is fixed | `PERF-HARNESS-DONE` needs one frozen world with fixed coordinates per zone before any run is comparable. Scaffolding is in `benchmarks/`; every `scenarios/zone-*.json` says `NOT DEFINED`. `scripts/build/new-benchmark-run.mjs` stamps identity and refuses a dirty tree or the wrong GPU |
| **Read `C9` and `C10` out of the first shaders-off run** | **#117 / #127** | free — the baseline requires shaders off anyway | Two log lines during a run Phase 1 performs regardless: does the 2 GB vertex-buffer growth stop with shaders off, and what does `Flywheel Backend:` read |
| **Candidate A test sequence (`PERF-UPFG-046`)** | 🔴 **UNTRACKED** | needs the GPU fixed, then a baseline | **#109/#111**. Two artifacts are built: `[Optional] Super Resolution(Upscaling).zip` (31 MB, drop into the profile you already play) for trying it, and `-nvidia-upscaling.zip` (214 MB, clean profile) for the actual A/B. Shader OFF and ON separately, TaCZ artifacts, FG at 2× only. `C-UPFG-01/02/03` are open and unfilled |
| **Candidate B is built and nothing is known about it** | 🔴 **UNTRACKED** | needs the GPU fixed; then one launch answers the first question | **#115**, `C-UPFG-09`. `[Optional] MCDLSSG(DLSS Frame Gen).zip` (22 MB) exists. Upstream is a **pre-release with 3 downloads on a 1-star repo**, Windows-only, and its `mods.toml` says MIT while GitHub says "Other". **Does it launch** is the whole first question. Never at the same time as A or C (`C-UPFG-08`) |
| **Candidate C is built, and V-Sync must be turned off before it is tested** | 🔴 **UNTRACKED** | one settings change, then a launch | **#115**, `C-UPFG-10`. `[Optional] PFG(Frame Gen).zip` (59 KB) exists. The author says cap render FPS below refresh and turn **V-Sync off**; the client currently runs `enableVsync: true` / `maxFps: 260`, so testing it as-is measures the interaction, not the mod. Toggle key `K`. Answers `PERF-UPFG-007` — does the simple shader-only implementation beat the big framework |
| **Nothing stops a player installing two candidates at once** | 🔴 **UNTRACKED** | accepted, mitigated in docs | **#115**, `C-UPFG-08`. Verified: no candidate declares an incompatibility with any other, so Forge will load two happily. The rule lives on the first screen of each `README.txt` and in the build's own cross-contamination check. The bad case is not a crash — it is that it works badly and the cause is unattributable |
| **Two `PERF-UPFG-031` fields still unknown** | 🔴 **UNTRACKED** | one launch answers both | **#109**. Where Super Resolution caches the DLSS model it downloads from `ngx.download.nvidia.com`, and whether uninstalling cleans it up. Also unobserved: what it does with no network |
| **The DLSS model resolves “latest”** | 🔴 **UNTRACKED** | affects benchmark validity | **#109**, `C-UPFG-00`. The model can change between runs without the pack changing, which breaks `PERF-UPFG-021`'s held-fixed requirement. Record the resolved object from the log in every A/B row |
| **🔒 `PERF-FREEZE` — no further performance mod enters the pack** | 🟡 **ACTIVE RULE** | lifts when the baseline exists AND all four candidates are assessed individually | **#99**. Covers Fast Noise, Let Me Despawn and Alternate Current explicitly. Does not block removing a candidate, changing a config, or fixing a bug. It exists because #97 installed four candidates one phase early and nothing was written down to stop it |
| **The performance work, in order — do not reorder** | 🟡 | steps 5–9 need a client | **#99**. `1.` canonical spec merge ✅ #96 · `2.` `PERF-*` ids replace `§N` ✅ #96 · `3.` `performance-baseline.md` exists ✅ #95 · `4.` freeze the roster ✅ #99 · `5.` **CLIENT LAUNCH** · `6.` fill the baseline against `a62adb9` · `7.` test each of the four individually (B1–B4) · `8.` test all four together (B5) · `9.` assign KEEP / REJECT / CONDITIONAL · `10.` only then consider Fast Noise / Let Me Despawn / Alternate Current. **Steps 1–4 are done; step 5 is the standing blocker** |
| **BadOptimizations — meet its acceptance criteria** | 🔴 **UNTRACKED** | needs a client | **#97**. `PERF-CLIENT-BADOPTIMIZATIONS` requires *no rendering corruption · no compatibility regression · measurable or neutral frame-time result*. Installed at `2.4.1`; none of the three is checked. It is `client`, so no server boot can test it |
| **AllTheLeaks — the 30 min / 2 h / 4 h runs** | 🔴 **UNTRACKED** | needs a long session | **#97**. `PERF-MEM-ALLTHELEAKS` requires heap growth, GC frequency, FPS degradation, MSPT drift and post-world-reload memory at three durations. Also: **keep ingredient-dedupe-style features OFF** until proven safe for this pack — inspect the generated config at first boot |
| **Dynamic FPS — confirm it does not throttle a hosted session** | 🔴 **UNTRACKED** | needs a client | **#97**. It reduces resource use while unfocused, which is the point — but a player hosting a LAN world who alt-tabs is running a server in that process. Verify the idle behaviour does not degrade TPS for the others |
| **Legendary Block Entities — test with resource packs** | 🔴 **UNTRACKED** | needs a client | **#97**. `PERF-RENDER-LEGENDARY-BLOCK-ENTITIES` names Embeddium, ModernFix and resource packs specifically. It rebakes chests, signs, bells, beds, shulkers and decorated pots into block models — a CT or texture pack that assumes the old renderer is exactly where this breaks |
| **Attribute the #97 batch** | 🔴 **UNTRACKED** | needs the baseline, then four isolation runs | **#97**. Four mods landed in one commit against `PERF-METHOD-ONEVAR`. Reference commit `a62adb9`. Until one is measured alone, no `keep` verdict may be written for any of them |
| **Establish the performance baseline** | 🔴 **UNTRACKED** | needs a launched client — the standing blocker | **#93/#94/#95**. `PERF-PRIORITY` item 2, and items 3–10 are all gated on it. The form is written and empty: `docs/performance-baseline.md`. spark is installed (#94) so the instrument is no longer the blocker; the run is |
| **Fill `docs/performance-benchmarks.md`** | 🔴 **UNTRACKED** | needs the baseline above | **#95**. Zones A–H, the TaCZ stress test, long-session runs and the worldgen tournament all have empty tables waiting. `PERF-METHOD-ONEVAR`: one changed variable per row, and `inconclusive` is a legal verdict |
| **Resolve the five recorded performance conflicts** | 🔴 **UNTRACKED** | each names its own settling run | **#95**. `docs/performance-conflicts.md` C1–C5: the Oculus/Embeddium mixin trade, the `midnightlib` version that lost, OptiFine (prevented), Improved Mobs × Brimm (cosmetic), and Create reporting `flywheel:off` — the last of which nobody has explained |
| **Measure what Oculus costs when no shader is selected** | 🔴 **UNTRACKED** | needs an FPS baseline, which needs a client | **#91**. Oculus sets three Embeddium mixins to `false` — `render.world.sky`, `render.entity`, `render.gui.font` — whether or not a shaderpack is on. Shipping the loader is a trade, and the size of the trade is unknown. Measure with the loader present/absent and no shader selected, before and after |
| **Decide on Rubidium (Embeddium) Extra** | 🔴 **UNTRACKED** | a developer decision, evidence already gathered | Modrinth `oY2B1pjg`, `embeddium_extra 0.5.4.4+mc1.20.1-build.131`, LGPL-3.0, Forge `[47.1.3,)`, `embeddium` at `versionRange = "*"`, all deps `side = "CLIENT"`. Adds 51 option labels; Embeddium 0.3.31's own lang file has **zero** keys for particles/fog/clouds/weather/sky, so it does not duplicate. **It optimises nothing by itself** — every gain comes from a player switching something off. Now specified: *Performance Spec `PERF-RENDER-EMBEDDIUM-EXTRA`* = `ALLOWED / OPTIONAL / NOT INSTALLED`. Allowed is not install-now; the install decision is deferred to the client phase where the option UI can be looked at (#93) |
| **Metafile comments do not survive `packwiz update`** | 🔴 **UNTRACKED** | none — decide where reasoning lives | `mods/in-control.pw.toml` carries a comment block explaining why its `side` must stay `both` (#88). packwiz rewrites a metafile on update and will drop it. The durable home is `docs/side-classification.md`, which `verify` enforces — either accept the comment as a bonus that may vanish, or stop writing them |
